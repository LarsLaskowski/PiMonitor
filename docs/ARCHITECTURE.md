...
`throttled`, `sysInfo`, `updates`, `uptime`, `processes` — each in its own file (`cpu.go`, `memory.go`, ...) with a `Collect()`
...
- **Fast tick** (`fastTick`, default every `poll_interval_seconds` = 5s): CPU usage/load
  average/temperature/hwmon sensors/throttling/memory/swap/disk/disk I/O/network/wireless. Every sub-collector's error is
